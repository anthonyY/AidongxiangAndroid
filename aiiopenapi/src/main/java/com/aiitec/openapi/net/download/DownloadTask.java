...
public class DownloadTask extends Handler {
    ...
    private final int THREAD_COUNT = 2;//线程数
    ...
    public void handleMessage(Message msg) {
        ...
        switch (msg.what) {
            case MSG_PROGRESS://进度
                ...
                mListner.onProgress(progress, mFileLength, (int) (progress * 100 / mFileLength));
                break;
            case MSG_PAUSE://暂停
                childPauseCount++;
    ...
    public synchronized void start() {
        try {
            LogUtil.e(TAG, "start: " + isDownloading + "\t" + mPoint.getUrl());
            if (isDownloading) {
                return;
            }
            isDownloading = true;
...
//                e.printStackTrace();
    ...
    /**
     * 因为一开始isDownloading就等于true了，暂停还是true,所以再加pause 和cancel来判断
     * @return
     */
    public boolean isDownloading() {
        return isDownloading && !pause && !cancel;
    }
